...
class PIE_Parser:
    ...
    def parse(self, classname, pattern, text):
        ...
        print "\n----\n".join(re.findall(self.pattern, text, re.S | re.I))
        
        return re.sub(self.pattern, self._sub_regex(classname, pattern), text, re.S | re.I)

    #receives each region of text not yet matched and runs the real regex against it
    def _sub_regex(self, name, pattern):
        print name, pattern
        def repl(match):
            return match.group(0).replace(match.group(1), re.sub(pattern, self._markup(name), match.group(1), re.S | re.I))
        return repl
